(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("25168d7a-c8ce-484e-99cd-153cc727d2de");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '25168d7a-c8ce-484e-99cd-153cc727d2de' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.1.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"c14d3d70-9e6c-43fc-aade-fb45135b32c3":{"defs":[],"roots":{"references":[{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18066","type":"Circle"},{"attributes":{},"id":"18084","type":"Selection"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18038","type":"Line"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"18026","type":"PolyAnnotation"},{"attributes":{},"id":"18022","type":"UndoTool"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"18057"},"selection_policy":{"id":"18056"}},"id":"18036","type":"ColumnDataSource"},{"attributes":{"data":{},"selected":{"id":"18059"},"selection_policy":{"id":"18058"}},"id":"18042","type":"ColumnDataSource"},{"attributes":{},"id":"18003","type":"DataRange1d"},{"attributes":{"axis":{"id":"18009"},"coordinates":null,"group":null,"ticker":null},"id":"18012","type":"Grid"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18037","type":"Line"},{"attributes":{},"id":"18057","type":"Selection"},{"attributes":{"children":[{"id":"18090"},{"id":"18088"}]},"id":"18091","type":"Column"},{"attributes":{},"id":"18059","type":"Selection"},{"attributes":{},"id":"18058","type":"UnionRenderers"},{"attributes":{},"id":"18001","type":"DataRange1d"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18067","type":"Circle"},{"attributes":{"source":{"id":"18064"}},"id":"18069","type":"CDSView"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"18070","type":"Title"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"18063"}]},"id":"18062","type":"Legend"},{"attributes":{},"id":"18014","type":"BasicTicker"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18043","type":"Line"},{"attributes":{},"id":"18017","type":"ResetTool"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18039","type":"Line"},{"attributes":{},"id":"18083","type":"UnionRenderers"},{"attributes":{"axis":{"id":"18013"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"18016","type":"Grid"},{"attributes":{},"id":"18054","type":"BasicTickFormatter"},{"attributes":{"below":[{"id":"18009"}],"center":[{"id":"18012"},{"id":"18016"},{"id":"18062"}],"height":500,"left":[{"id":"18013"}],"output_backend":"webgl","renderers":[{"id":"18040"},{"id":"18046"},{"id":"18068"}],"title":{"id":"18070"},"toolbar":{"id":"18027"},"toolbar_location":null,"width":500,"x_range":{"id":"18001"},"x_scale":{"id":"18005"},"y_range":{"id":"18003"},"y_scale":{"id":"18007"}},"id":"18000","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"18051","type":"BasicTickFormatter"},{"attributes":{"toolbar":{"id":"18089"},"toolbar_location":"above"},"id":"18090","type":"ToolbarBox"},{"attributes":{"source":{"id":"18042"}},"id":"18047","type":"CDSView"},{"attributes":{"source":{"id":"18036"}},"id":"18041","type":"CDSView"},{"attributes":{},"id":"18056","type":"UnionRenderers"},{"attributes":{"tools":[{"id":"18017"},{"id":"18018"},{"id":"18019"},{"id":"18020"},{"id":"18021"},{"id":"18022"},{"id":"18023"},{"id":"18024"}]},"id":"18027","type":"Toolbar"},{"attributes":{"coordinates":null,"data_source":{"id":"18036"},"glyph":{"id":"18037"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18039"},"nonselection_glyph":{"id":"18038"},"view":{"id":"18041"}},"id":"18040","type":"GlyphRenderer"},{"attributes":{},"id":"18023","type":"SaveTool"},{"attributes":{},"id":"18005","type":"LinearScale"},{"attributes":{"overlay":{"id":"18025"}},"id":"18019","type":"BoxZoomTool"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18065","type":"Circle"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"18025","type":"BoxAnnotation"},{"attributes":{"coordinates":null,"formatter":{"id":"18054"},"group":null,"major_label_policy":{"id":"18055"},"ticker":{"id":"18010"}},"id":"18009","type":"LinearAxis"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18045","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18044","type":"Line"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"18046"}]},"id":"18063","type":"LegendItem"},{"attributes":{"overlay":{"id":"18026"}},"id":"18021","type":"LassoSelectTool"},{"attributes":{"coordinates":null,"formatter":{"id":"18051"},"group":null,"major_label_policy":{"id":"18052"},"ticker":{"id":"18014"}},"id":"18013","type":"LinearAxis"},{"attributes":{"coordinates":null,"data_source":{"id":"18042"},"glyph":{"id":"18043"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18045"},"nonselection_glyph":{"id":"18044"},"view":{"id":"18047"}},"id":"18046","type":"GlyphRenderer"},{"attributes":{"data":{},"selected":{"id":"18084"},"selection_policy":{"id":"18083"}},"id":"18064","type":"ColumnDataSource"},{"attributes":{"callback":null},"id":"18024","type":"HoverTool"},{"attributes":{"coordinates":null,"data_source":{"id":"18064"},"glyph":{"id":"18065"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18067"},"nonselection_glyph":{"id":"18066"},"view":{"id":"18069"}},"id":"18068","type":"GlyphRenderer"},{"attributes":{},"id":"18010","type":"BasicTicker"},{"attributes":{},"id":"18055","type":"AllLabels"},{"attributes":{},"id":"18018","type":"PanTool"},{"attributes":{},"id":"18052","type":"AllLabels"},{"attributes":{},"id":"18020","type":"WheelZoomTool"},{"attributes":{},"id":"18007","type":"LinearScale"},{"attributes":{"children":[[{"id":"18000"},0,0]]},"id":"18088","type":"GridBox"},{"attributes":{"toolbars":[{"id":"18027"}],"tools":[{"id":"18017"},{"id":"18018"},{"id":"18019"},{"id":"18020"},{"id":"18021"},{"id":"18022"},{"id":"18023"},{"id":"18024"}]},"id":"18089","type":"ProxyToolbar"}],"root_ids":["18091"]},"title":"Bokeh Application","version":"2.4.1"}}';
                  const render_items = [{"docid":"c14d3d70-9e6c-43fc-aade-fb45135b32c3","root_ids":["18091"],"roots":{"18091":"25168d7a-c8ce-484e-99cd-153cc727d2de"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();